...
void DrawIDHookWindow()
{
	...
	{
		ImGui::Checkbox("Esp Only ID Hook", &cvar.esp_idhook_only);
		...
		for (unsigned int i = 0; i < items.size(); i++)
		{
			...
			if (strstr(item.name, "[T]")) col = ImColor(colorList.get(0)->onebased_r, colorList.get(0)->onebased_g, colorList.get(0)->onebased_b, 1.0f);
			if (strstr(item.name, "[CT]")) col = ImColor(colorList.get(1)->onebased_r, colorList.get(1)->onebased_g, colorList.get(1)->onebased_b, 1.0f);
			if (strstr(item.name, "[On]")) col = ImColor(colorList.get(2)->onebased_r, colorList.get(2)->onebased_g, colorList.get(2)->onebased_b, 1.0f);
			if (strstr(item.name, "[Off]")) col = ImColor(colorList.get(6)->onebased_r, colorList.get(6)->onebased_g, colorList.get(6)->onebased_b, 1.0f);
