...
void JumpBug(float frametime, struct usercmd_s *cmd)
{
	...
	if(cvar.kz_jump_bug_auto && g_Local.flFallVelocity>=404.8f)
		if(g_Local.flHeightorigin-(g_Local.flFallVelocity*frametime/cvar.wav_speed *15)<=0)
			autojb=true;
...
void DrawLongJump()
{
	...
	ImGui::Begin("Longjump", reinterpret_cast<bool*>(true), ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoInputs);
	{
		...
		if (cvar.kz_show_kz && g_Local.bAlive && CheckDraw() && g_pEngine->GetMaxClients())
		{
			...
			{
				ImVec2 label_size = ImGui::CalcTextSize("Start", NULL, true);
				ImVec2 label_size2 = ImGui::CalcTextSize("Stop", NULL, true);

				if (CalcScreen(g_Local.vT1, ft1) && CalcScreen(g_Local.vT2, ft2))
				{
					ImGui::GetCurrentWindow()->DrawList->AddLine({ ft1[0] + 1, ft1[1] + 1 }, { ft2[0] + 1, ft2[1] + 1 }, ImColor(colorList.get(5)->onebased_r, colorList.get(5)->onebased_g, colorList.get(5)->onebased_b, cvar.esp_alpha));
					ImGui::GetCurrentWindow()->DrawList->AddLine({ ft1[0], ft1[1] }, { ft2[0], ft2[1] }, ImColor(colorList.get(2)->onebased_r, colorList.get(2)->onebased_g, colorList.get(2)->onebased_b, cvar.esp_alpha));
				}

				if (CalcScreen(g_Local.vT1, ft1))
					ImGui::GetCurrentWindow()->DrawList->AddText({ ft1[0] - label_size.x / 2, ft1[1] - 14 }, ImColor(colorList.get(4)->onebased_r, colorList.get(4)->onebased_g, colorList.get(4)->onebased_b, cvar.esp_alpha), "Start");

				if (CalcScreen(g_Local.vT2, ft2))
					ImGui::GetCurrentWindow()->DrawList->AddText({ ft2[0] - label_size2.x / 2, ft2[1] - 14 }, ImColor(colorList.get(4)->onebased_r, colorList.get(4)->onebased_g, colorList.get(4)->onebased_b, cvar.esp_alpha), "Stop");
			}
		}
		ImGui::End();
		ImGui::PopStyleColor();
	...
	if (cvar.kz_show_kz && g_Local.bAlive && CheckDraw() && g_pEngine->GetMaxClients())
	{
		...
		ImGui::Begin("kz-show-kz", NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize);
		{
			...
			if (g_Local.flDamage >= g_Local.iHealthStrafe)
				col = ImColor(colorList.get(0)->onebased_r, colorList.get(0)->onebased_g, colorList.get(0)->onebased_b, 1.0f);
...
#define TraceEdge(x,y){\
	...
	pmtrace_s* t1 = g_pEngine->PM_TraceLine(end,start,1,g_Local.iUsehull,-1);\
