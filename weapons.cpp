...
void UpdateWeaponData()
{
	if (g_Local.bAlive)
	{
		...
		case WEAPON_P228:
			...
			else if (g_Local.flVelocity2D > 0)
		...
		case WEAPON_SCOUT:
			...
			else if (g_Local.flVelocity2D > 170)
		...
		case WEAPON_AUG:
			...
			else if (g_Local.flVelocity2D > 140)
		...
		case WEAPON_AWP:
			...
			else if (g_Local.flVelocity2D > 10)
...
void ItemPreFrame(struct local_state_s *from, struct local_state_s *to, struct usercmd_s *usercmd, int runfuncs, double time, unsigned int random_seed)
{
	if (runfuncs)
	{
		...
		{
			if (g_Local.bAlive) delay = GetTickCount();
...
void DrawWeaponName()
{
	...
	ImGui::Begin("DrawWeapon", reinterpret_cast<bool*>(true), ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoInputs);
	{
		...
		if (CheckDraw() && g_pEngine->GetMaxClients())
		{
			...
			if (cvar.misc_weapon_local && g_Local.pEnt->curstate.weaponmodel && g_Local.bAlive && GetTickCount() - delay < 5000)
			{
				ImVec2 label_size = ImGui::CalcTextSize(GetWeaponName(g_Local.pEnt->curstate.weaponmodel), NULL, true);
				ImGui::GetCurrentWindow()->DrawList->AddRect({ 20 - 2, (float)y + 52 }, { label_size.x + 24, (float)y + 66 }, ImColor(colorList.get(5)->onebased_r, colorList.get(5)->onebased_g, colorList.get(5)->onebased_b, cvar.esp_alpha), cvar.esp_rounding);
				ImGui::GetCurrentWindow()->DrawList->AddRect({ 20 - 3, (float)y + 51 }, { label_size.x + 23, (float)y + 65 }, ImColor(cvar.color_red, cvar.color_green, cvar.color_blue, cvar.esp_alpha), cvar.esp_rounding);
				ImGui::GetCurrentWindow()->DrawList->AddText({ 20, (float)y + 50 }, ImColor(cvar.color_red, cvar.color_green, cvar.color_blue, cvar.esp_alpha), GetWeaponName(g_Local.pEnt->curstate.weaponmodel));
			}
		}
		ImGui::End();
		ImGui::PopStyleColor();
